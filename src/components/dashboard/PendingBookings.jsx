...
const PendingTenants = () => {
  ...
  <table  className='w-full bg-white border-collapes border-spacing-2 border border-slate-400 table-auto'>
    ...
    <tbody >
      {
        pendingTenants?.length===0? <tr className='dashboard-headings text-center w-full'> <td colSpan="9">No List Available </td></tr>:
  pendingTenants&&pendingTenants.map((tenant,index)=>(
    <tr key={index}>
